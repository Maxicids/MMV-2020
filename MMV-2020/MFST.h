...
#define MFST_TRACE_START_LOG(LOG)	*logfile.stream << std::setfill(' ') << std::endl << std::setw(25) << ' ' << " СИНТАКСИЧЕСКИЙ АНАЛИЗ " << std::endl << std::endl \
									<< " Шаг" << "   " << std::setfill(' ') \
...
typedef MyStack<short> MFSTSTACK;													// стек автомата
...
namespace MFST
{
	...
	struct Mfst																			// магазинный автомат
	{
		...
		MFSTSTACK st;																	// стек автомата
		MyStack<MfstState> storestate;												// стек для сохранения состояний 
		Mfst();
		Mfst(
